...
private int[] legendSize= new int[4];


public MapWindow(java.util.List<String> pathList, HashMap<String,BuildingStructure> buildings, java.util.List<String[]> settings,String time, int size) {
	...
	// Won't set Settings if it has already been sit.
...
private void setSettings(){
	...
	legendSize[0] = Integer.parseInt(settings.get(4)[0]);
	legendSize[1] = Integer.parseInt(settings.get(4)[1]);
	legendSize[2] = Integer.parseInt(settings.get(4)[2]);
	legendSize[3] = Integer.parseInt(settings.get(4)[3]);




	// Removes these indexes so that the remaining icons can be iterated through.
	settings.remove(4); settings.remove(3); settings.remove(2); settings.remove(1); settings.remove(0);
}
...
private void drawMapLegend(Graphics g2) {
	...
	// Draws parameter
	g2.setColor(Color.BLACK);
	g2.fillRect(LX-3, LY-3, legendSize[2]+6, legendSize[3]+6 );
	g2.setColor(Color.WHITE);
	g2.fillRect(LX, LY, legendSize[2], legendSize[3]);
